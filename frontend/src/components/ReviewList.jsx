            ...
            <Card.Body>
                <Card.Title className="text-rigth h1">Your Reviews ({!reviews.length ? 0 : reviews.length})</Card.Title>
                    <ul className="d-grid gap-3 mb-3 p-3 mx-auto" style={{ width: '100%' }}>
                        ...
                        {    
                            !userRepo.currentUser().restaurantID ? <CardHeader>{ x.restaurantID }</CardHeader> : <CardHeader>{ x.userID }</CardHeader>   
                        }                        
...
// export const RestaurantReviewList = props => {
//     const userRepo = new UserRepository();
//     const restRepo = new RestaurantRepository();
//     const [reviews, setReviews] = useState(undefined);
...
//     useEffect(() => {
//         let restID = userRepo.currentUser().restaurantID;
//         setReviews(restRepo.getRestaurantReviews(restID));
//         // console.log(reviews)
//     }, []);
...
export const ReviewList = () => {
    ...
    //return restaurantOwner ? <RestaurantReviewList /> : <UserReviewList />;
}
